# 源码学习记录
[how to write markdown code](https://blog.csdn.net/qq_36528377/article/details/127804828)
## classical code
+ [resnet](https://github.com/yangfa-zhang/source-code-learning/blob/main/resnet.py)
+ [RNA starter](https://github.com/yangfa-zhang/source-code-learning/blob/main/RNA%20starter.py)
+ [USPNet(LSTM+attention)](https://github.com/yangfa-zhang/source-code-learning/blob/main/uspnet-remake/Net/LSTM.py)
+ [BLSTM module by hand](https://github.com/yangfa-zhang/source-code-learning/blob/main/BLSTM%20module%20by%20hand.ipynb)
## bluecast(a light-weight and fast auto-ml library)
+ [how to use bluecast](https://github.com/yangfa-zhang/source-code-learning/blob/main/multi-class%20prediction%20of%20obesity%20risk/README.md)
## computer vision
+ [detection using transformer(detr)](https://github.com/yangfa-zhang/source-code-learning/blob/main/detr.md)
+ [super resolution and reconstruction of remote image(diffusion model)](https://github.com/yangfa-zhang/source-code-learning/blob/main/diffusion-model-for-super-resolution.md)
## ai4bio
+ [USPNet(a signal peptide predictor using RNA sequence data)](https://github.com/yangfa-zhang/source-code-learning/blob/main/uspnet-remake/readme.md)
+ [NucleicNet(a framework to predict binding preference of RNA constitunents on protein surface)](https://github.com/yangfa-zhang/source-code-learning/blob/main/NucleicNet-remake/readme.md)
