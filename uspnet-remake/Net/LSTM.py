...
class BLSTM(nn.Module):
    ...
    #由LSTM层输出的特征作为输入，并返回经过注意力加权后的特征
    def attention_net_with_w(self, lstm_out):
        '''
        :param lstm_out: [batch_size, n_step, n_hidden * num_directions(=2)]
        :return:
        '''
        #用torch.chunk函数将LSTM的输出分割成两部分，
        #得到两个张量，每个张量的形状为[batch_size, n_step, n_hidden]
        ...
        #双线性矩阵乘法（torch.bmm）计算注意力上下文表示，存储在atten_context中
        #这里m作为权重，atten_w.transpose(1, 2)是转置操作
        atten_context = torch.bmm(m, atten_w.transpose(1, 2))

        #将权重转换为概率分布
        softmax_w = F.softmax(atten_context, dim=-1)

        #h与softmax_w相乘，得到加权后的上下文表示
        context = torch.bmm(h.transpose(1,2), softmax_w)
        #得到一个加权平均的特征表示，形状为[batch_size, hidden_dims]
